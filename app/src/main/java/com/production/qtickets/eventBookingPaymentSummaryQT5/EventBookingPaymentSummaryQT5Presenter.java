...
public class EventBookingPaymentSummaryQT5Presenter implements EventBookingPaymentSummaryQT5Contracter.Presenter{
    ...
    @Override
    public void BookTickets(String bookingXML,boolean isDTCM) {
        try{
                        ...
                        public void onResponse(
                            ...
                            if (mView != null) {
                                if (response.isSuccessful() &&  response.body() != null) {
                                    mView.BookTickets(response.body(),isDTCM);
                                } else {
    ...
    @Override
    public void createUserForDTCM(RequestBody bookindxml) {

        try{

        //    Response{protocol=http/1.1, code=400, message=Bad Request, url=https://apie.q-tickets.com/api/DTCM/CreateCustomer}
            if (mView != null)mView.showPb();
          //  RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), new Gson().toJson(bookindxml));
            APIClient.getClientQT5().create(APIInterface.class)
                    .createDTCMEvent(AppConstants.Authorization_QT5,"application/json",bookindxml)
                    .enqueue(new Callback<CreateDTCMUserModel>() {
                        @Override
                        public void onResponse(Call<CreateDTCMUserModel> call, Response<CreateDTCMUserModel> response) {
                            if(mView != null){
                                if(response.isSuccessful() && response.body()!=null){
                                    mView.setUserCreateDTCMData(response.body());
                                }
                            } else {
                                mView.onFailure(call, null, this, null);
                            }
                        ...
                        @Override
                        public void onFailure(Call<CreateDTCMUserModel> call, Throwable t) {
                            if (mView != null) {
                                mView.dismissPb();
                                mView.onFailure(call, t, this, null);
                            }
