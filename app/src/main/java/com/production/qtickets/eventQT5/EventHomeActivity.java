...
public class EventHomeActivity extends AppCompatActivity implements View.OnClickListener, EventHomeContracter.View {
    ...
    String event_name = "", selcategoryname = "";
    ...
    private ArrayList<CustomModel> heading = new ArrayList<>();
    ...
    ImageView iv_nodataavailable;
    SharedPreferences prefs;

    private String currentTab = "eventscreen";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        iv_nodataavailable = findViewById(R.id.iv_nodataavailable);
        ...
        if (b != null) {
            if (b.getParcelableArrayList("headerList") != null) {
                heading = b.getParcelableArrayList("headerList");
            }
        }
        if (getIntent().getExtras() != null) {
            ...
            if (getIntent().getExtras().getString("category_name") != null)
                selcategoryname = getIntent().getExtras().getString("category_name");

            Log.d("26sep", "Receiving" + selcategoryname + " selected_cat_id" + selected_cat_id);
...
//      sliderLayout  = (SliderLayout) findViewById(R.id.slider);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.fb_filter: {
                ...
                i.putExtra("category_id", selected_cat_id);
            ...
            case R.id.ll_events: //26sep

                if (currentTab.equals("eventscreen")) {
                    return;
                }
                Intent intentss = new Intent(this, EventHomeActivity.class);
                intentss.putExtra("categoryId", "15");
                intentss.putParcelableArrayListExtra("headerList", heading);
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == MY_REQUEST_CODE) {
                if (data != null) {
                    ...
                    selcategoryname = data.getStringExtra("category_name");

                    Log.d("26sep", "onActivityResult : " + selcategoryname);

                    if (selected_cat_id.equals("")) {
                        presenter.getCarouselBannersAgain(sessionManager.getCountryID(), "-1");
                    } else {
                        presenter.getCarouselBannersAgain(sessionManager.getCountryID(), selected_cat_id);
                    }
    ...
    private void setupViewPager(String event_name, WrapContentHeightViewPager viewPager,
                                String cat_id, Integer selected_day, List<AllCategoryQT5Data> data,
                                Integer min_price, Integer max_price, Intent datsss) {
        ...
        if (datsss != null) {
            ...
            if (datsss.getStringArrayExtra("category_id") != null) {
                ...
                editor.putString("category_name", selcategoryname);
            }
...
//            setBanner(carouselBanners.data);
            if (carouselBanners.data != null && !carouselBanners.data.isEmpty()) {
                iv_nodataavailable.setVisibility(View.GONE);
...
//                presenter.getCountry();
            ...
            iv_nodataavailable.setVisibility(View.VISIBLE);
            CarouselBannerAdapter carouselBannerAdapter = new CarouselBannerAdapter(carouselBanners.data, EventHomeActivity.this);
            recycler.setAdapter(carouselBannerAdapter);
...
//            setBanner(carouselBanners.data);
            ...
            Log.d("26sep", "setCarouselBannersAgain ELSEEEEEEEEEEEEE");
    ...
    public void setAllCategories(AllCategoryQT5Model allCategoryQT5Model) {
        if (allCategoryQT5Model.statusCode.equals("200")) {
            ...
            if (!Objects.requireNonNull(selcategoryname).isEmpty()) {
                for (int i = 0; i < allCategoryQT5Model.data.size(); i++) {
                    if (selcategoryname.equals(allCategoryQT5Model.data.get(i).name)) {
            ...
            int temp_selected_category = -1;
            if (selected_cat_id != null) {
                if (selected_cat_id.equals("")) {
                    temp_selected_category = -1;
                } else {
                    temp_selected_category = Integer.parseInt(selected_cat_id);
                }
            }

            Log.d("26sep", "selected_cat_id" + selected_cat_id);

            //hitting 26sep
            presenter.getCarouselBanners(sessionManager.getCountryID(), String.valueOf(temp_selected_category));
    ...
    public void getCarouselBanners() {
        presenter.getCarouselBannersAgain(sessionManager.getCountryID(), String.valueOf(-1));
    }
...
//      spinner_country.setSelection(0, false);

        spinner_country.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {

                Log.d("11oct", "onItemSelected spinner-country" + data.get(position).codeAlpha2);
                Log.d("11oct", "onItemSelected spinner-country" + data.get(position).name);
                Log.d("11oct", "onItemSelected spinner-isdCode " + data.get(position).isdCode);
                ...
                setupViewPager(event_name, viewPager, selected_cat_id, selected_days, allCategoryQT5Modeltemp, selected_min_price, selected_max_price, getIntent());
